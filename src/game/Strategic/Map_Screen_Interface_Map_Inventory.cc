...
static UINT32 MergeRefillableSectorInventory(std::vector<WORLDITEM>& items)
{
	...
	for (size_t iFirst = 0; iFirst < items.size(); ++iFirst)
	{
		...
		if (uiNeeded >= uiObjects) continue; // already packed as tightly as it goes
...
void StackAndSortMapInventoryPool(void)
{
	...
	MapScreenMessage(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE,
		...
		ST::format("Sector inventory sorted: {} gun(s) unloaded, {} attachment(s) removed, {} object(s) merged, {} stack(s) combined.",
			uiGunsUnloaded, uiDetached, uiObjectsMerged, uiSlotsMerged) :
		ST::string("Sector inventory sorted."));
}
...
void RefillMercItemsFromMapInventoryPool(void)
{
	...
	{
		MapScreenMessage(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, ST::string("Point at the item to refill from first."));
		return;
	}
	...
	{
		MapScreenMessage(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE,
			ST::format("{} cannot be used to refill anything.", item->getName()));
		return;
	}
	...
	{
		MapScreenMessage(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE,
			ST::format("Nothing to refill with {} here.", item->getName()));
		return;
	}

	MapScreenMessage(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE,
		ST::format("Refilled {} {} ({} points).", uiObjectsTopped, item->getName(), uiPointsMoved));
